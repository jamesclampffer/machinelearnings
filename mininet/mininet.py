...
class MiniResBlock(nn.Module):
    ...
    def __init__(self, chan_in: int, chan_out: int, reduce: bool = False, activation : nn.Module = nn.SiLU):
        super().__init__()
        self.seq = nn.Sequential(
            DepthwiseSeparableConvBlock(chan_in, chan_out, stride=2 if reduce else 1, activation=activation),
